<template>
  <div>
    <img class="mt-2 mb-2" alt="I love you" src="../../assets/byt.jpg" width="600" height="400">
          ...
          <div class="col-6">
            <div class="text-center p-3 border-round-sm border-primary border-2 font-bold">
              <div v-for="question in answers" :key="`woman-${question.key}`" class="flex align-items-center">
                <RadioButton v-model="womanAnswer" :inputId="`woman-${question.key}`" name="dynamic" :value="question.name"
                             class="mb-2"/>
...
export default {
  ...
  data() {
    return {
      question: 'Как вы оба относитесь к бытовым обязанностям в доме?',
      ...
      womanAnswer: '',
      answers: [
        {name: 'Делим обязанности поровну и стараемся придерживаться этого распределения', key: '1'},
        {name: 'Тот, кто свободен, тот и делает больше', key: '2'},
        {name: 'Постоянно меняемся обязанностями, чтобы никому не было скучно', key: '3'},
        {name: 'У нас есть расписание, и мы стараемся его придерживаться', key: '4'},
        {name: 'Все делает одна половинка', key: '5'},
      ]
    }
  },
  methods: {
